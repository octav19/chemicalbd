"""
Tests for the `alpha_monocarbonyl_alkylation` function from `chemicalbd.bond_disconnector` module.

The `alpha_monocarbonyl_alkylation` function is tested under different scenarios to ensure its correctness.
...
#Test in the case the pattern is absent
def test_alpha_monocarbonyl_alkylation_absent():
    """
    Test the behavior of `alpha_monocarbonyl_alkylation` function when the pattern is absent.
    ...
    assert alpha_monocarbonyl_alkylation(mol) == []

#Test in the case the function receives an input different from a mol object
def test_alpha_monocarbonyl_alkylation_invalid ():
    """
    Test the behavior of `alpha_monocarbonyl_alkylation` function with invalid input.
...
#Test in the case the pattern is present
def test_alpha_monocarbonyl_alkylation_present():
    """
    Test the behavior of `alpha_monocarbonyl_alkylation` function when the pattern is present.
    ...
    None
    """
    mol = Chem.MolFromSmiles('CC(=O)C(C)CC')
    with patch('IPython.display.display') as mock_display: #display is a function for IPython; as the tests are done in Python, the function is transformed
        ...
        reactant_list = alpha_monocarbonyl_alkylation(mol) #List of reactants that could form the pattern that was disconnected 
    ...
    assert Chem.MolToSmiles(Chem.MolFromSmiles('CCOC(=O)C(CC)C(C)=O')) in smiles_list
    ...
    assert Chem.MolToSmiles(Chem.MolFromSmiles('CCOC(=O)C(C)(CC)C(C)=O')) in smiles_list
    assert Chem.MolToSmiles(Chem.MolFromSmiles('CCOC(=O)C(C)C(C)=O')) in smiles_list
